...
def second_gui_show_results(firstLoaded, d):
    ...
    while(True):
        ...
        if firstLoaded == True and event == 'Distance to the centroid, for the first cluster':
            if os.path.exists(dirResultsOutput):
                ...
                if d is not None:
                    if os.path.isfile(dirResultsOutput + "distances_firstCluster" +  d_str + ".png"):
                   ...
                   #      window['-DIST_FIRST_CLUSTER-'].update(data = bio.getvalue())
                        ...
                        img = cv2.imread(dirResultsOutput + "distances_firstCluster" +  d_str + ".png")
    ...
    #        break   ## secondLoaded == True and
        ...
        if secondLoaded == True and event == 'Distance to the centroid, for the second cluster':
            if os.path.exists(dirResultsOutput):
                ...
                if d is not None:                    
                    if os.path.isfile(dirResultsOutput + "distances_secondCluster" +  d_str + ".png"):
                        img = cv2.imread(dirResultsOutput + "distances_secondCluster" +  d_str + ".png")
...
def gui_show_results(clusteringRes, execTime, numberImg, d):
    d_str = ""
    if d is not None:
        d_str += "_1_" + str(d)
  ...
  ##          window.FindElement['-LISTBOX_FEATURES-'].update(values = lst)
        ...
        if event == 'PCA graph output for the first two features':  
            ...
            if os.path.exists(dirResultsOutput):
                ...
                if d is not None:
                    if os.path.isfile(dirResultsOutput + "pca_graph" + d_str + ".png"):
                        image = Image.open(dirResultsOutput + "pca_graph" + d_str + ".png")
                        image.thumbnail(MAX_SIZE)
        ...
        if (firstLoaded == True) and  event == 'Distances to the centroid': ##  or ind_data == len(data_to_save) - 1  ##  firstLoaded == True and 
             ...
             second_gui_show_results(firstLoaded, d)
...
def post_proc_pfs_only():
    ...
    if True:         
        ...
        if numberTests == 1:
              ...
              while True:
                  ...
                  if True:
                      ...
                      first_clustering_storing_output = dest_path + first_clustering_storing_output
                     
                      f_new = ""
                      for inf, f in enumerate(first_clustering_storing_output):
                          if f != ' ' and f != '\t' and f != '\n' or (inf == 0 and f != ':'):
                            f_new += f
                      ...
                      for indf2, f in enumerate(f_new):
                         if f == ':' and indf2 == 0:
                             countf += 1
                             f_new = f_new[1:]
                      ...
                      f_new = f_new[:t] + f_new[(t+1):]
                          
                      first_clustering_storing_output = f_new     
                      adder = 0
                      while True:
                          if os.path.exists(first_clustering_storing_output + '/'):
                              first_clustering_storing_output += '_' + str(adder)
                              adder += 1
                          else:
                              break
     ...
     ##                 clusteringInfData, executionTime, totCountImages = videoAnalysis(curTest, numberTests, infi, data_from_tests)               
                      ...
                      print("Here full video: " + infi[5] + infi[7] + "\\" + "test_000.mp4")
                      ...
                      if output_video_filenames is None:
                          clusteringInfData, executionTime, totCountImages = videoAnalysis_single(infi, infi[5] + infi[7] + "\\" , n_images)
                          datax = (clusteringInfData, executionTime, totCountImages)  
                          gui_show_results(clusteringInfData, executionTime, totCountImages, None)
                          ...
                          while True:       
                             ...
                             ##             if ind_inf != len(infx) - 1: 
                                         if curTest < numberTests-1:
                                             ...
                                             data_from_tests = videoAnalysisMultipleVideosWithinOne(curTest, numberTests, infi, data_from_tests, this_dir, dirVideo, frame_rate, limToCompare, buffer_imgs)
                ...
                ##                             infx += 1 
                                         ...
                                         #       break
                                             ...
                                             print("first_clustering_storing_output" + first_clustering_storing_output) 
                                           ...
                                           ##      print("Number of tests: " + str(numberTests))
                                                 ...
                                                 clustering_output = videoAnalysisMultipleVideosWithinOne(curTest, numberTests, infi, data_from_tests, this_dir, dirVideo, frame_rate, limToCompare, buffer_imgs)
                 ...
                 ##                cv2.waitKey(0)
                                         ...
                                         ################################
                                             ...
                                             if len(data_to_save[0]) != 3:
                                                  print(data_to_save)
                                                  print("Ok")
                              
                                             clusteringResList = data_to_save[-1]
                                             ...
                                             for c in clusteringResList:
                                         ################################
                                         
                              # clusteringRes, execTime, numberImg = data_to_save[0]
                              ...
                              ################################
                                                   ...
                                                   if len(c) == 5:
                                                       ...
                                                       gui_show_results(c, execTime, numberImg, None)
                                                   else:
                                                       print("Error")
                                                       break
 ...
 ##           else:  
          ...
          #      for ind_inf, infi in enumerate(infx):
                    ...
                    while True: 
                        ...
                        while True:
                            ...
                            if True:      
                                ...
                                firstNew = ""
                                for p in first_clustering_storing_output:
                                    if not(p == '\t' or p == '\n' or p == ' '):
                                        firstNew += p
                                ...
                                for indP, p in enumerate(firstNew):
                                    if p == ':' and indP != 1:
                                        indIntrus = indP
                                
                                first_clustering_storing_output = firstNew[:indIntrus] + firstNew[(indIntrus+1):]
                ...
                ##             if ind_inf != len(infx) - 1: 
                              ...
                              #      infi[len(infi)-2] = first_clustering_storing_output
                                    infi = (decisorLevel, mainPathVideoData, sequence_name, dest_path, mtsVideoPath, mp4VideoFile, IFVP, locationMP4_file, roiPath, newRoiPath, first_clustering_storing_output, pathPythonFile)
...
##                cv2.waitKey(0)
         ...
         #           if len(data_to_save[0]) > 0: 
                ...
                if len(data_to_save[0]) == 3:
                    clusteringRes, execTime, numberImg = data_to_save[0]
                    gui_show_results(clusteringRes, execTime, numberImg, None)
                else: 
                    for d in range(len(data_to_save[0])):
                        clusteringRes, execTime, numberImg = data_to_save[0][d]     
                                               
                        gui_show_results(clusteringRes, execTime, numberImg, d)
