...
def basler_configs(gap_bet_tests, repeat, dict_data_basler, dirStorage, numberTests, startTime, enab_timestamps):
...
##   print("Press any leter from the keyboard to continue ...")
   ...
   if key == True:
       while True:
         ...
         ##      print(str(camera.IsGrabbing()))
          ...
          ##                     cv2.imwrite(base_path + "/image" + str(counter) + ".tiff", img)
               ...
               while True:
                   if os.path.exists(folderThisTest):
                       folderThisTest = base_path + '/Test_' + str(curTest) + '_' + str(indNew) + '/'
                       indNew += 1
                   else:
                       break
     ...
     #               cv2.imwrite(base_path + "/image" + str(ind_img) + ".tiff", img)
               ...
               img_array = []
               ind = 0
               for filename in glob.glob(folderThisTest + '/*.tiff'):      ## base_path2 + 'files_python/Img_track/*.tiff'
                   print("A")
                   if "image" in filename:
                       ...
                       print("Image " + str(ind) + " drained to set PP")
                       ind += 1
               ...
               print("Video " + 'test_' + '00' + str(counterTest) + '.avi')
               
               if os.path.isfile('test_' + '00' + str(counterTest) + '.avi'):
               ...
               if numberTests == 1:
                    ...
                    while True:
                        ...
                        if True:
                            ...
                            clusteringInfData, executionTime, totCountImages = videoAnalysis_single(infi, False, folderThisTest, ind)
                           ...
                           while True: 
                               ...
                               if True:
                                   ...
                                   if ind_inf != len(infx) - 1:
                                       ...
                                       data_from_tests = videoAnalysis(ind_inf, numberTests, infi, data_from_tests) 
                                       ...
                                       clustering_output = videoAnalysis(ind_inf, numberTests, infi, data_from_tests) 
...
def read_pfs_file(gap_bet_tests, numberTests, repeat, startTime, enab_val_pfs, enab_timestamps):
 ...
 ##       for i in range(0,numberTests-1):    
       ...
       while not go:
        ...
        ##    with open("file.pfs", "r") as input:
           ...
           ##     listNamesFeatures.append()
            ...
            # get balck thresh data
            # get format_image data
            black_thresh = 2
            
            width = listValuesFeatures[-4]
            height = listValuesFeatures[-3]
            
            composeD = '(' + str(width) + 'x' + str(height) + ')'
            ...
            for indr, r in enumerate(res_dims):
                if r == composeD:
                    restype_sel = res_dims[indr] 
            
            format_image = restype_sel
            
            params_stream_basic = [listValuesFeatures[-2], black_thresh, format_image, listValuesFeatures[-1], listValuesFeatures[4], listValuesFeatures[5], listValuesFeatures[7], listValuesFeatures[6], listValuesFeatures[3]]
            
            get_test_image(params_stream_basic)
            
            layout = [[sg.Button("Ok")], [sg.Button("Hold")],[sg.Button("Exit")]]
            
            window = sg.Window("Check parameters", layout)
       ...
       basler_configs(gap_bet_tests, repeat, dict_data_pfs, dirStorage, numberTests, startTime, enab_timestamps)           
