...
def videoAnalysis_single(tupleForProcessing, folderThisTest, count, video_given = False):  
  ...
  #      parent_dir = dest_path    
    ...
    ##    pre_dirInit = "C:\\Users\\Other\\files_python\\py_scripts\\ffmpeg-5.0.1-full_build\\bin\\GUI\\"
        pre_dirInit = os.path.dirname(sys.argv[0])
        ...
        if os.path.exists(roi_path):
            ...
            if roi_pathx[-2] == '_':
               ...
               roi_pathx = roi_pathxt + str(endInd) + '_'                
                ...
                roi_pathx = roi_pathx + str(endInd) + '_' 
            roi_path = roi_pathx + '/'          
    ...
    count = int(count)
    print("Here: " + folderThisTest)  
    
    count_slashes = 0
    
    for ind_f, f in enumerate(folderThisTest):
        if f == "\\":
            count_slashes += 1
    ...
    if count_slashes > 10:
        ...
        for ind_f, f in enumerate(folderThisTest):
            if f == "\\":
                bslaches.append(ind_f)
        
        folderThisTest = folderThisTest[:bslaches[-4] + 1]
    ...
    for video_image in range(0,count):        
            ...
            image = cv2.imread(folderThisTest + 'image' + str(video_image) + ".tiff")    
            ...
            if image is not None:
                ...
                p = cv2.imwrite(roi_path + "/roi_image%d.jpg" % video_image, roi_imx)  
          #      print("Result: " )
          #      print(p)
                indNow = video_image
    ...
    print("Roi path: " + str(roi_path))
    
    totCountImages = indNow  
  ...
  #  count = 0
    ...
    print("fixed_roi_path: " + str(fixed_roi_path))
        ...
        if os.path.exists(fixed_roi_path):
            ...
            if fixed_roi_pathx[-2] == '_':
               ...
               fixed_roi_pathx = fixed_roi_pathxt + str(endInd) + '_'                
                ...
                fixed_roi_pathx = fixed_roi_pathx + str(endInd) + '_' 
  ...
  #  totCountImages = count
    ...
    for video_image in range(0,totCountImages-1):  
...
def videoAnalysis_single_justFeatures(tupleForProcessing, folderThisTest, count, video_given = False):  
  ...
  #      parent_dir = dest_path    
    ...
    folderThisTest_sp = folderThisTest.split('/')
    
    folderThisTestAnother = ''
    for indP, p in enumerate(folderThisTest_sp):
        if indP < 9:
            folderThisTestAnother += p + '/'
    
    folderThisTest = folderThisTestAnother + 'Test_0' + '/'
    ...
    for video_image in range(0,count):        
            ...
            print(folderThisTest + 'image' + str(video_image) + ".tiff")
...
#        print(imx)
    ...
    print(newListMetrics)
            
    for ind_new, metricNew in enumerate(newListMetrics):
            print("ind_new: " + str(ind_new))
            for indInsideMetric, valueInMetric in enumerate(metricNew):
